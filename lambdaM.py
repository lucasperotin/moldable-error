...
lambdas=["1e-8","1e-7","2e-7","5e-7","1e-6"]
lambdasInt=[1e-8,1e-7,2e-7,5e-7,1e-6]
print(lambdas)
...
print(t_data_length)
print(a_data_length)
plt.plot(lambdasInt,data_length,label=r'\textsc{List} / \textsc{Lpt}',marker='o',linewidth=2,color='red')
plt.plot(lambdasInt,b_data_length,label=r'\textsc{Batch} / \textsc{Lpt}',marker='o',linestyle='-',color='blue')
if model != "amd":
	plt.plot(lambdasInt,t_data_length,label=r'\textsc{MinTime} / \textsc{Lpt}',marker='o',linestyle='-',color='green',linewidth=1)
#plt.plot(lambdasInt,a_data_length,label=r'\textsc{MinArea} / \textsc{Lpt}',marker='o',linestyle='-',color='yellow')
plt.xlabel(r"$\lambda$")
plt.ylabel(r"Makespan")
plt.xlim([1e-8,1e-6])
plt.xscale("log")
...
if big:
	plt.savefig("figs/lambdasM_500_"+model+"_"+str(set_no)+"_"+str(p)+".pdf")
	print("figs/lambdasM_500_"+model+"_"+str(set_no)+"_"+str(p)+".pdf saved.")
else:
	plt.savefig("figs/lambdasM_100_"+model+"_"+str(set_no)+"_"+str(p)+".pdf")
	print("figs/lambdasM_100_"+model+"_"+str(set_no)+"_"+str(p)+".pdf saved.")
plt.clf()
